...
class ClientEventHub:
    ...
    async def _targets(
        self, thread_id: str | None, source_client_id: str | None = None
    ) -> list[object]:
        async with self.lock:
            if thread_id:
                subscribed = [
                    route.socket
                    for route in self.clients.values()
                    if thread_id in route.threads
                ]
                if subscribed:
                    return subscribed
            if source_client_id and source_client_id in self.clients:
                return [self.clients[source_client_id].socket]
            return [route.socket for route in self.clients.values()]
    ...
    async def broadcast(
        ...
        for socket in await self._targets(thread_id, source_client_id):
        ...
        if turn_id:
            ...
            async with self.lock:
                if key in self.completed_turns:
                    return False
                source = self.turn_sources.setdefault(key, source_client_id)
